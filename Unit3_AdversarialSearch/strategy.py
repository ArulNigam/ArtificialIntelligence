...
Original file is located at
    https://colab.research.google.com/drive/1HwPrGhTrsHdWHFycI4wka8_UNb72FvPm
"""
...
class Strategy():
    ...
    def string2board(self, strrep):
        board = [8][8]
        for i in range(0,8):
            for j in range(0,8):
                board[i][j] = strrep[((i + 1) * 10) + j + 1]
        return board
    ...
    def board2string(self, board):
        ...
        for i in range(8):
            for j in range(8):
                strrep[((i + 1) * 10) + j + 1] = board[i][j]
        return strrep

    def convert(self, list):
        return ((list[0] + 1) * 8) + list[1] + 1

    def best_strategy(self, board, player, best_move, still_running):
        best_move.value = self.convert(self.best_strategy_helper(self.string2board(board), player))
    ...
    def best_strategy_helper(self, board, color):
        ...
        if self.first_turn:
            ...
            ret, notret = self.evaluate(board, color, possible_moves)
            return ret
