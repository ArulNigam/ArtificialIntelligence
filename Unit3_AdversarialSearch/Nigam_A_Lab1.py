...
def utility(turn, tc, state):
    ...
    for line in tc:
        if state[line[0]] == state[line[1]] and state[line[1]] == state[line[2]]:
            if state[line[0]] == turn:
                return 1
            elif state[line[0]] in ['O', 'X']:
                return -1
    return 0
...
def main():
    X = raw_input("X is human or AI? (h: human, a: AI)")
    O = raw_input("O is human or AI? (h: human, a: AI)")
    state = '' + raw_input("input state (ENTER if it's an empty state): ")
